  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      float map(float3 p)
      {
        ...
        const float d = sdf_box(p, float3(0.0f, 0.0f, 3.0f), float3(1.0f, 0.5f, 0.5f), quat_euler(float3(0.0f, kSixthPi, kFifthPi)));
        const float e = sdf_capsule(p, float3(1.0f, 1.0f, 2.0f), float3(-1.0f, -1.0f, 2.5f), 0.3f);
        const float f = sdf_cylinder(p, float3(-1.0f, 1.0f, 0.0f), float3(1.0f, -1.0f, 0.5f), 0.3f);

        float k = 0.5f;
        float res = sdf_uni_smooth(a, b, k);
        res = sdf_uni_smooth(c, res, k);
        res = sdf_uni_smooth(d, res, k);
        res = sdf_uni_smooth(e, res, k);
        res = sdf_uni_smooth(f, res, k);
